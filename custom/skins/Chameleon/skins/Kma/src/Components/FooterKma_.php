...
//print_r($this->getLinkListItems());
		...
		foreach ( $sidebar as $key => $items ) {
			array_walk( $items, static function ( &$value, $key ) use( $skinTemplate ) {
				$value = $skinTemplate->makeListItem( $key, $value );
			} );

			if ( count($items ) ) {
				$columnsTitle[] = "<div class=\"col-md-3 mt-4\"><h5 class=\"font-weight-bold text-uppercase\">$key</h5><ul class=\"list-unstyled\">"
...
$toolbox = $skinTemplate->get( 'sidebar' )[ 'TOOLBOX' ] ?? array();
 ...
 //print_r($toolbox);
		...
		foreach ( $toolbox as $key => $linkItem ) {
			...
			$text = $linkItem['text'] ?? $skinTemplate->getSkin()->msg( $linkItem['msg'] ?? $key )->text();
// var_dump($link);
...
$this->getSkin()->getOutput()->enableOOUI();
		...
		$ret .= '<footer class="page-footer">';
			...
			$ret .= '<div class="container ">';
				$ret .= $this->indent( 1 );
				$ret .= '<!-- Grid row -->';
				$ret .= '<div class="row py-4 footer-row1 text-center justify-content-center align-items-top">';
...
/*
$ret .= new \OOUI\DropdownInputWidget( [
			// 'name' => $this->mName,
...
<select id="toolbox_select"><option value="" disabled selected>' . $skinTemplate->getMsg( 'toolbox' )->escaped() . '</option>' . 
implode ( array_map( function ( $value ) {
	return '<option value="' . $value['href'] . '">' . $value['text'] . '</option>';
}, $arr) )
...
/*
				$ret .= '<nav><ul><li class=""><a class="" href="#!">toolbox</a><ul class="nav-dropdown">';
				$ret .= implode( $this->getLinkListItems() );
				$ret .= '</ul></li></ul></nav>';
*/


/*
				$ret .= $this->wrapDropdownMenu( 'toolbox', implode( $this->getLinkListItems( 2 ) ) );
*/
...
/*
			...
			$ret .= '<div class="row pt-4 text-center footer-row2 justify-content-center align-items-top">';
					...
					$ret .= '<div class="col-md-3" mt-4>';
						$ret .= $this->indent( 1 );
						$ret .= '<img style="margin-right:-46px" height="150"src="https://fina.knowledge.wiki/footer-logo.png" />';
					$ret .= '</div>';
