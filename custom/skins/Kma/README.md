...
* Copy the files from this repository into a folder named "Kma" in the "skins" folders of your wiki.
* copy the files in the SITE directory into the Mediawiki root folder, then add in LocalSettings.php the following:

```
wfLoadExtension( 'Bootstrap' );
wfLoadSkin( 'Kma' );
$wgDefaultSkin = 'kma';
$egChameleonEnableVisualEditor = true;  // (optional)

$egChameleonLayoutFile= __DIR__ . '/FINA-layout.xml';
$egChameleonExternalStyleModules = [ __DIR__ . '/f_bootswatch.scss' ];
$egChameleonThemeFile = __DIR__ . '/f_variables.scss';
```
  
(the original Chameleon skin should be also installed)
